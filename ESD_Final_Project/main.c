...
static const char* CWMODE = "AT+CWMODE=3\r\n";
static const char* STATIC_IP = "AT+CIPSTA=\"192.168.43.253\"\r\n";
static const char* MULTIPLE_CON_ON = "AT+CIPMUX=1\r\n";
static const char* TCP_CREATE = "AT+CIPSERVER=1,80\r\n";
static const char* JOIN_AP = "AT+CWJAP_CUR=\"OnePlus 6T\",\"password\"\r\n";
static const char* CONN_STATUS = "AT+CIPSTATUS\r\n";
static const char* IP_DATALEN = "AT+CIPSEND=0,756\r\n";
...
void Ultrasonic_GetData()
{
    ...
    __disable_irq();
    distance = ultra_read/(58 * 2);     // converting ECHO lenght into cm
    __enable_irq();
...
void ESP_Send(const char* ATCmd, uint16_t len)
{
            ...
            if(processflag == 1)
            {
                ...
                switch(*match)
               {
                ...
                case 'T':
                    ...
                    {
                        /* Retake temperature? Display on screen */
                    }
...
void PORT2_IRQHandler(void)
{
    ...
    if(P2->IFG & BIT7)
    {
        ...
        else
        {
            ...
            ultra_read = millis*1000 + TIMER_A0->R;
